...
HEADER = f'''export function Header({{ date, gameCount }}: {{ date: string; gameCount: number }}) {{
  return (
...
GAMECARD = f'''import {{ formatAmericanOdds, getFavoriteSide }} from "@/lib/odds";
...
PAGE = f'''import {{ BestBetsPanel }} from "@/components/BestBetsPanel";
...
Path("components/Header.tsx").write_text(HEADER, encoding="utf-8")
Path("components/GameCard.tsx").write_text(GAMECARD, encoding="utf-8")
Path("app/page.tsx").write_text(PAGE, encoding="utf-8")
print("done")
